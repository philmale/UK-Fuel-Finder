        ...
        try:
            ...
            if resp.status_code in (429, 500, 502, 503, 504):
                ...
                if attempt < retries:
                    ...
                    debug_print(f"All retries exhausted, failing with status {resp.status_code}")
...
For Home Assistant, run multiple instances with different coordinates:
  command: "python3 /config/uk_fuel_finder.py --lat 51.5074 --lon -0.1278 --radius-miles 10"
    ...
    location_group = parser.add_argument_group('location (required except for --healthcheck or --station-id)')
    ...
    radius_group = parser.add_argument_group('radius (choose one, required except for --healthcheck or --station-id)')
    radius_exclusive = radius_group.add_mutually_exclusive_group()
    radius_exclusive.add_argument("--radius-miles", type=float, 
                                 help="Search radius in miles")
    radius_exclusive.add_argument("--radius-km", type=float, 
    ...
    # Validate: if not healthcheck or station-id or full-refresh-only, require location args
    # Note: station-name DOES require location (too many results without filtering)
    # full-refresh can be standalone (just refreshes cache) or combined with a query
    if not args.healthcheck and not args.station_id and not (args.full_refresh and args.lat is None):
        if args.lat is None or args.lon is None:
            parser.error("--lat and --lon are required (unless using --healthcheck, --station-id, or --full-refresh alone)")
        if args.radius_miles is None and args.radius_km is None:
            parser.error("one of --radius-miles or --radius-km is required (unless using --healthcheck, --station-id, or --full-refresh alone)")
...
def normalise_config(cfg: dict) -> dict:
    ...
    out = cfg.copy()
    for k in ("stations_baseline_days", "stations_incremental_hours", "prices_incremental_hours"):
...
def fetch_all_batches(token, path, params=None):
    ...
    while True:
        ...
        debug_print(f"Batch {batch}: received {len(batch_data)} records (total: {len(all_results)})")
    ...
    debug_print(f"Fetched {len(all_results)} total records in {elapsed:.1f}s ({batch} batches)")
...
def build_station_data(station_id, station, all_prices, fuel_list, home_lat=None, home_lon=None):
    """
...
def process_prices_to_dict(prices_data):
    ...
    for record in prices_data:
        ...
        for fuel in fuel_prices:
            ...
            # Fix data entry errors: if price < 2p, it's likely in pounds (multiply by 100)
            if price < 2.0:
                old_price = price
                price = round(price * 100, 1)
                debug_print(f"Fixed price error: {old_price}p -> {price}p for {fuel_type} at station {node_id[:8]}...")
...
def build_output(nearby_stations, all_prices, fuel_types, max_stations=None, sort_by_price=False, mode="location", home_lat=None, home_lon=None, metadata=None, cache_state=None):
    """
    ...
    if sort_by_price:
        ...
        result.sort(key=lambda x: (
            x.get(f"{first_fuel}_price") if x.get(f"{first_fuel}_price") is not None else float('inf'),
            x.get("distance_miles", float('inf')) if home_lat and home_lon else x.get("name", "")  # Secondary sort
        ))
    ...
    for fuel_type in fuel_list:
        ...
        if cheapest:
            ...
            debug_print(f"Cheapest {fuel_type}: {cheapest['price']}p at {cheapest['name']}")
    ...
    if force_baseline or needs_baseline_refresh(state):
        ...
        if needs_stations_incremental(state):
            ...
            if stations_data:
                ...
                state["all_uk_stations"] = merge_stations(state["all_uk_stations"], updates)
                updated = True
...
def main(argv=None):
    ...
    if config_path.exists():
        ...
        if DEBUG:
            debug_print(f"Config file not found at default location, using hardcoded defaults")
    ...
    if args.healthcheck:
        ...
        try:
            ...
            if stations_inc:
                stations_dt = datetime.fromisoformat(stations_inc.replace("Z", "+00:00"))
            ...
            if DEBUG:
                debug_print(f"Baseline age: {baseline_age_days:.1f} days (limit: {baseline_days_limit})")
                debug_print(f"Stations incremental age: {stations_age_hours:.1f} hours (limit: {stations_inc_limit})")
                debug_print(f"Prices incremental age: {prices_age_hours:.1f} hours (limit: {prices_inc_limit})")
                debug_print(f"Total stations: {len(state.get('all_uk_stations', {}))}")
                debug_print(f"Stations with prices: {len(state.get('all_uk_prices', {}))}")
            
            # Determine health
            is_healthy = (
                baseline_age_days < baseline_days_limit and
                stations_age_hours < stations_inc_limit * 2 and  # Allow 2x grace period
                prices_age_hours < prices_inc_limit * 2
            )
    ...
    # Standalone full-refresh mode (no query, just refresh cache)
    if args.full_refresh and args.lat is None:
        if DEBUG:
            debug_print("=== Standalone Full Refresh Mode ===")
        ...
        try:
            ...
            try:
                ...
                if DEBUG:
                    debug_print("Forcing full baseline refresh...")
                state, _ = update_cache(token, state, force_baseline=True)
                save_state(state)
                
                if DEBUG:
                    debug_print(f"Cache refreshed: {len(state.get('all_uk_stations', {}))} stations, {len(state.get('all_uk_prices', {}))} with prices")
                
                print(json.dumps({
                    "success": True,
                    "total_stations": len(state.get("all_uk_stations", {})),
                    "stations_with_prices": len(state.get("all_uk_prices", {})),
                    "cache_updated": state.get("prices_last_incremental"),
                    "cache_baseline": state.get("stations_baseline_at")
                }))
                sys.exit(0)
    ...
    if args.station_id:
        ...
        try:
            ...
            if DEBUG:
                debug_print(f"Found {output['found']} station(s), {output['errors']} error(s)")
    ...
    if args.station_name:
        ...
        try:
            ...
            if DEBUG:
                debug_print(f"Searching for stations matching pattern: '{search_pattern}'")
            ...
            try:
                ...
                print(json.dumps({"error": f"Invalid regex pattern: {e}"}), file=sys.stderr)
                sys.exit(1)
            ...
            for station_id, station in nearby_stations.items():
                ...
                if (pattern.search(station_name) or 
                    pattern.search(station_brand) or 
                    pattern.search(station_postcode)):
            ...
            metadata = {
                ...
                "radius_miles": round(radius_km / 1.609344, 1) if args.radius_miles else None,
    ...
    if DEBUG:
        debug_print(f"Location: ({args.lat:.4f}, {args.lon:.4f}), radius={radius_display}")
    ...
    try:
        ...
        try:
            ...
            state, cache_updated = update_cache(token, state, force_baseline=args.full_refresh)
        ...
        elapsed = time.time() - start_time
        debug_print(f"=== Complete in {elapsed:.1f}s - found {output['found']} stations with prices ===")
